...
const OrderCard = ({ order }: OrderCardProps) => {
  ...
  return (
    <div
      className="bg-wj-white border border-wj-border p-6 flex flex-col gap-5"
      ...
      {(order.items?.length ?? 0) > 0 && (
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
          {order.items?.slice(0, 4).map((item) => (
            <div
              key={item.id}
              className="aspect-square border border-wj-border overflow-hidden"
          ...
          {(order.items?.length ?? 0) > 4 && (
            <div className="aspect-square border border-wj-border flex items-center justify-center">
              <span className="font-body text-[12px] text-wj-muted">
                +{(order.items?.length ?? 0) - 4}
              </span>
            </div>
          )}
