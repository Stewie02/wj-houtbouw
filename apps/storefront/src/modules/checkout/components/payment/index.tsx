...
const Payment = ({
  ...
  const stripeProvider = availablePaymentMethods.find((m) => isStripeLike(m.id))
  ...
  useEffect(() => {
    ...
    initiatePaymentSession(cart, { provider_id: stripeProvider.id }).catch((err) => {
      ...
      <div className={isOpen ? "flex flex-col gap-4" : "hidden"}>
        {paidByGiftcard ? (
          ...
          <p className="font-body text-[14px] text-wj-muted">
            Online payment is not available at this time. Please contact us to complete your order.
          </p>
        ) : (
          stripeReady ? (
